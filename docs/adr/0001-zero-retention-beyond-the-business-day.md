...
Vajra runs the current Business Day and nothing else. At Rollover, the End of Day Report is exported as a file on disk, then all transactional data, Drafts, and operational logs from the closed day are purged. The Product Master, Customer Master, and the new Opening Stock survive — that is all.
