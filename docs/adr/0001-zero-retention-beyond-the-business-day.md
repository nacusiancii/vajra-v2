...
Vajra runs the current Business Day and nothing else. At Rollover, the End of Day Report is exported as a file on disk, then all transactional data, Drafts, Journals, and operational logs from the closed day are purged. The Product Master, Customer Master, and the new Opening Stock survive — that is all.
...
- Drafts are day-scoped operational state, not master data. Approving Rollover wipes them with the transactional ledger after confirm — no carry-forward into the next Business Day (ADR-0010).
- Journals are day-scoped notes, not master data (ADR-0011). Approving Rollover wipes them with the day's transactional data. Issue #164 has no EOD bridge for Journals; a later ticket adds that file.
